...
- `docs/internal/history.md` 记录了完整 Phase A~H 交付过程
...
- [x] 生成 v2.0 移除清单文档（`docs/internal/v2.0_removal_checklist.md`）

### E.2 bmap[] 标记 deprecated

`docs/internal/history.md` 记录 v1.9 已将 `bmap[]` 标记
`[[deprecated]]`，但 UNIF loader 仍使用它。

- [x] 确认 `bmap[]` 的 deprecated 标注是否已落地 — **未落地**。
  `bmap[]` 在 `ines_init.cpp:30` 仍被活跃使用，不能标记 deprecated。
  `docs/internal/history.md` 的声称有误。详见 `v2.0_removal_checklist.md` §3。
