...
> **前置文档**：`docs/history/checklists/FCEUX11-1.16_KagamiQA-遗留问题与构建难题.md`（下称「遗留文档」）
> **关联**：`docs/history/reports/FCEUX11-1.16_KagamiQA-{审计报告,修复验证报告}.md`、`docs/history/plans/FCEUX11-1.16_KagamiQA-{PLAN,P5-权威性构建计划}.md`、`docs/BuildGuide.md`
...
| **0-3 ✅ `b53a7a0`** | 校正遗留文档 §三与审计链路中已过期的 3 项记载（本文档 §一） | `docs/history/checklists/FCEUX11-1.16_KagamiQA-遗留问题与构建难题.md` | 「2MB 全零」「后缀 bug 未修」「rlib ABI」三处更正 |
...
- **E-1**：题干描述的「周期过冲」**不成立** —— 插桩实测 VBL `period=6820`，398/398 帧完全一致。
  ...
  **独立的时序参数**，不是单一线性偏移。已有两次失败尝试（1-cycle NMI delay 被证伪并 revert）。
  ...
  完整记录见 `docs/history/surveys/e1_vbl/FCEUX11-1.16_E-1-VBL调查记录.md` 与 `docs/history/surveys/e1_vbl/`。
...
- **后续路线**见 `docs/history/plans/FCEUX11-Stage3-权威性迭代与通用化路线.md`——回答「能否迭代趋于权威、再追求通用化」。其中 §3.2 的「冻结泄漏纪律」**在 Stage-2 期间即生效**：不再向共享 schema 添加领域专用字段、不再向 `SutAdapter` 添加方法。该纪律零成本，但决定了未来抽象重构的可行性。
