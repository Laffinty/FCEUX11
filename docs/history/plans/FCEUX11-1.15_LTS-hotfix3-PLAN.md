...
>
> > **注**：`docs/history/reports/v1.15_hotfix3_phase_b_diagnostics.md` 未单独生成——Phase B 诊断已在上游 overview（`v1.15_hotfix3_overview.md`）中覆盖，且本 PLAN §三自身即为可执行诊断文档。Phase B 的 6 个 commit message 中均含完整变更说明和验证结果，可替代独立诊断报告。
>
...
> | **C-5** | ⏭ Skip。**PLAN §四 C-5 的两条诊断假设与现状不符**：<br>① `mmc3.cpp:316-318` 用 `&=` 不是 `=`、`PRGmask8[]` 是 `uint32_t[32]` 不是 `uint8`、调用方 42 处全传 KiB 值，结果三行实际是 **no-op**（不是"全 1 → 越界"）。<br>② `mmc5.cpp:1037-1038` null 守卫在当前 `FCEU_gmalloc → FCEU_abort → abort()` 契约下**不可达**。<br>见 `docs/history/reports/v1.15_hotfix3_phase_c_diagnostics.md` §五。 | — |
>
...
- [ ] Phase 完成报告（`docs/history/reports/v1.15_hotfix3_phase_*.md`）
