...
## 9. Step 4 尝试：runppu(3) → runppu(6)（回滚，2026-08-01 下午）

**假设**：NMI_ENTRY cycle 阶梯 rows 4-9 = 16,20,24,28,32,36（每行 +4 dots），runppu(3) 只让 rows 0-3 到 2。加大到 runppu(6)（2 个 LDX 周期）可能让 rows 4-9 也 +1。

**结果**：vbl_05 X 序列 `[3,2,2,2,2,2,2,1,1,1]`
- rows 1-6 到 2（比 runppu(3) 多 3 行）
- **row 0 超调到 3**（新问题）
- rows 7-9 仍 1
- `$6000` 仍 0x01 FAIL

**关键规律**（三个版本对比，证明单参数无法统一）：
...
**每次 +3 dots 整体上移 1，但 blargg 10 行 per-row 相位漂移导致永远有行超调（=3）或欠调（=1）**。单参数 runppu(N) 无法让 10 行同时 =2——证实 vbl_05 根因是 **VBL flag 可见相位 vs blargg 测试循环的 per-row 漂移**，不是 NMI 延迟量。

**处置**：回滚到 runppu(3)（无超调的最优状态，rows 0-3 修复）。vbl_05 接受为**有据已知限制**（需修正 VBL flag 相位本身，超出单参数范围）。
