# FCEUX11 兼容性检测报告（v2.0_hotfix1）— 已存档

> **⚠️ 已存档（2026-08-24）**：本文档为 v2.0_hotfix1 兼容性测试的最终报告，已归档。  
> **请勿基于此文档执行构建任务。** 如需最新兼容性数据，请运行 `batch_compat_test` 工具重新扫描。  
> 后续迭代计划详见 `docs/plans/cycle_accurate_ppu_necessity.md`。
...
### 3.2 Mapper 通过率对比（确认 PPU/NMI 时序为统一根因）

| Mapper | 名称 | 总数 | PASS | Stuck | 通过率 |
|--------|------|------|------|-------|--------|
| 11 | Color Dreams | 29 | 29 | 0 | **100%** |
| 69 | Sunsoft FME-7 | 12 | 12 | 0 | **100%** |
| 71 | Camerica | 14 | 14 | 0 | **100%** |
| 4 | MMC3 | 1140 | 1030 | 110 | 90.4% |
| 3 | CNROM | 176 | 150 | 26 | 85.2% |
| **1** | **MMC1** | **643** | **524** | **119** | **81.5%** |
| 2 | UNROM | 334 | 270 | 64 | 80.8% |
| 66 | GxROM | 47 | 35 | 12 | 74.5% |
| 0 | NROM | 281 | 201 | 80 | 71.5% |
| 64 | Tengen RAMBO1 | 33 | 23 | 7 | 69.7% |
| 7 | AXROM | 50 | 30 | 20 | 60.0% |
| 19 | Namcot 106 | 58 | 34 | 24 | 58.6% |

**结论**: stuck 率与 mapper 复杂度无关，PPU/NMI 时序偏差是影响所有 mapper 的统一根因。Mapper 1 的 81.5% 通过率在主流 mapper 中处于中等水平。
...
| **Tier 4** | Mapper 1 (MMC1) 诊断（已完成） | +0（确认为 PPU 时序问题） | 2h | — |
| **Tier 5** | 零页 stuck (0x0000) NMI 向量修复 | +4 ROM | 2-3h | 低 |
| **深层** | PPU VBlank NMI 精确时序重写 | +100~200 ROM | 8-12h | 高 |

> **Tier 4 根因发现**: Mapper 1 的 119 个 stuck ROM 不是 mapper 特定问题。所有 mapper 的 stuck 率对比证实 PPU/NMI 时序偏差是统一根因。Mapper 1 的 81.5% 通过率优于 mapper 0 (71.5%) 和 mapper 7 (60%)。MMC1 写处理器的 busy check 逻辑正确。需要深层 PPU 修正。  
> **Tier 5 完成后预计兼容率可达 ~86.5%**。  
