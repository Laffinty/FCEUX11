...
> 后续收口会话推进：135/177 = 76.3%（`3e3e4ef` + `efe9678`）→ 140/177 = 79.1%
> （DMC 收口 `fc057e2`）→ **142/177 = 80.2%（2026-08-14 PPU open-bus + dummy-read
> 会话，首次诚实达 phase 6 ≥80% 门槛）**。见
> `docs/wip_2.0_plan/phase_6_integration.md` §9.1.0 与 README v0.15/v0.16。
...
| 阶段 | T1 门槛 | 含义 | 状态 |
|------|--------|------|------|
| **phase 6 收口** | **≥ 80%**（候选 A，2026-08-14 诚实达标） | 见 §3.3a | **诚实 80.2%（142/177）已过** |
| **phase 7 默认切换前** | ≥ 85% (150/177) | 默认 ON 的硬门槛 | 差 8 ROM（从 142/177 起） |
| **phase 8 清理后** | ≥ 90% (160/177) | mmc3 全部修复后达成 | 差 18 ROM（从 142/177 起） |
| **v2.1** | ≥ 95% (168/177) | 完整修复 deep model 后 | 差 26 ROM（从 142/177 起） |

#### 3.3a phase 6 门槛 3-tier 候选(候选 A 已诚实达标,2026-08-14)

| 候选 | 数值 | 当前差 | 优点 | 缺点 |
|------|------|--------|------|------|
| **A. ≥ 80%** | ≥ 80% | **已过(诚实 80.2% = 142/177)** | 与战略转向一致(不追 micro-drift);phase 6 收口 | 留 35 ROM 缺口给 phase 7/8 |
| **B. 追 85%** | ≥ 85% | 8 ROM | phase 7 收口可达成;不需修 mmc3 deep model | phase 6 收口还需 session 修 8 ROM(主要是 cpu + ppu) |
| **C. 坚持 90%** | ≥ 90% | 18 ROM | 与原计划一致 | 需起 mmc3 IRQ deep model session(1-2 周) |

**结论**:**候选 A 已于 2026-08-14 诚实达标**(142/177 = 80.2%,全量 Rust-primary 实测)。
理由:
- 战略转向(ADR-011)已明确"不追 micro-drift"
- mmc3 12 fail 是 mapper 模型完整性问题,不是 cycle 残差——属于 phase 7+ 范围
- 142/177 较 v1.16 +22 PASS 是**真实改善**,不应被抽象门槛挡住
...
|------|------|--------------|--------------|
| T1 blargg | **80.2% (142/177,§3.3 诚实 Rust-primary,2026-08-14 PPU open-bus + dummy-read 会话)** | **≥ 80%（候选 A，已达标）** | ≥ 85% |
