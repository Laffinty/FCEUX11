...
| `FCEUX11_RUST_PPU` | ON | Rust PPU 引擎是唯一实现（C++ PPU 引擎已在 v2.1.1.7 Step C 删除，`-DFCEUX11_RUST_PPU=OFF` 自该版本起是 configure-time 错误）。统一使用 `build-rust-ppu/`；历史遗留的 `build/` 目录须先删除或重新配置 |
