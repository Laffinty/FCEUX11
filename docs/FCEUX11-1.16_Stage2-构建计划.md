...
| PR | 内容 | 文件 | 验收 |
|---|---|---|---|
| **0-1** | 修复 `menu_slot_check` 的硬编码 Python 路径（N-1） | `tests/tests.json` | binary 改为 `python`；在无该路径的机器上不再 setup_error |
| **0-2** | 重新生成 `kagamiqa_migration_matrix.json`，用 `2d15b66` 后的 runner | 产物（gitignored） | 记录真实 passed/failed；若仍大面积 failed，逐条归因 |
| **0-3** | 校正遗留文档 §三与审计链路中已过期的 3 项记载（本文档 §一） | `docs/FCEUX11-1.16_KagamiQA-遗留问题与构建难题.md` | 「2MB 全零」「后缀 bug 未修」「rlib ABI」三处更正 |
| **0-4** | 修复 `docs/BuildGuide.md` §8.4 的死引用 | `docs/BuildGuide.md` | §8.4 指向的 `scripts/_find_vcvars.bat` 实际在 `scripts/archive/`；改为指向 `do_build.ps1` |
...
| **A-6** | 保险：对 `bus.cpp` 预防性加 `/GL-` | `src/CMakeLists.txt`（紧邻 `:593`） | `bus.h:197-198` 的 `aread_[0x10000]`+`bwrite_[0x10000]`（合计 1 MiB）是仅次于 `kSpriteIdxLUT` 的 LTCG 风险点 |
...
| PR | 内容 | 文件 | 验收 |
|---|---|---|---|
| **B-1** | 修复 `rshift` 为逻辑右移：`Ok((x as u32).wrapping_shr(n as u32) as i32)` | `src/rust/crates/fceux11-lua/src/bindings/bit.rs:38` | `rshift(-1,31)==1`；`rshift(256,8)==1` 不回归 |
| **B-2** | 修正 4 处测试期望值（F2~F5）为规范值，并在每行加注 LuaBitOp 规范引用 | `tests/lua_scripts/test_bit.lua:56,63,64,65` | 期望值有据可查，避免后人再次误判为实现 bug |
| **B-3** | 补齐 `tohex` 的负数 `n` → 大写语义（规范要求，当前实现**永不产出大写**） | `bit.rs:56-67` | 新增断言 `tohex(255,-2)=="FF"`、`tohex(-1,-8)=="FFFFFFFF"` |
| **B-4** | 对 `bit` 库全 API 做一次规范对照审计（`tobit/bnot/band/bor/bxor/lshift/arshift/rol/bswap`），补测试 | `bit.rs`、`test_bit.lua` | 防止同类「实现对/测试错」或反向问题继续潜伏 |
...
| PR | 内容 | 文件 | 验收 |
|---|---|---|---|
| **C-1** | 产出 staticlib：`crate-type = ["rlib","staticlib"]`（保留 rlib 供根 crate 依赖）**或**在 CMake 用 `cargo rustc --crate-type staticlib`（无需改 Cargo.toml） | `src/rust/crates/kagami-qa/Cargo.toml:7` 或 `tests/CMakeLists.txt` | 产出 `kagami_qa.lib` |
| **C-2** | 修正产物路径，补 triple 段；复用 `src/rust/CMakeLists.txt:36` 的既有写法 | `tests/CMakeLists.txt:661` | 路径命中实际产物 |
| **C-3** | 删除两个未定义变量，改为字面量 `ntdll userenv ws2_32 dbghelp`；补 `fceux11_drivers_common` 与 `src/`、`src/drivers/`、`src/drivers/null` 头文件目录（对齐 `fceux11_add_headless_test_executable`，`tests/CMakeLists.txt:47-73`） | `tests/CMakeLists.txt:663-664` | 无 LNK2019 |
| **C-4** | 消除 Rust std 符号重复风险（见下方「首要风险」） | `src/rust/Cargo.toml`、`tests/CMakeLists.txt` | 无 LNK2005 |
| **C-5** | 依赖关系改为增量正确形式：`add_custom_command(OUTPUT ... DEPENDS ...)` + `add_custom_target(... DEPENDS ...)`，替换裸 `add_custom_target`；统一 `CARGO_TARGET_DIR` 到 `src/rust` 用的那个 | `tests/CMakeLists.txt:637-643` | cargo 不再每次构建都重跑；两处 target dir 合一（当前会把依赖编译两遍） |
...
| PR | 内容 | 文件 | 验收 |
|---|---|---|---|
| **D-1** | 实测 `blargg_suite` 实际驱动的 ROM 数，产出「177 个 ROM 的接入状态清单」 | `tests/tests.json`、`docs/` | 得到真实覆盖率分母/分子，取代「22/174」等口径不一的说法 |
| **D-2** | 按 D-1 清单扩充 manifest 接入，目标 ≥80% ROM 覆盖 | `tests/tests.json` | 已知失败的 ROM 显式标注为 expected-fail，不得静默跳过 |
| **D-3** | 给 `ci.yml` 补 `dtolnay/rust-toolchain@stable` 步骤（对齐 `kagami-qa.yml:46`） | `.github/workflows/ci.yml` | 消除「靠 runner 预装 Rust 侥幸通过」的脆弱性 |
| **D-4** | 打通 `kagami-qa.yml` 的 matrix 步骤，产出可信 migration matrix 并作为 CI artifact 上传 | `.github/workflows/kagami-qa.yml` | matrix 不再是本地手工产物；权威性指标可被 CI 复算 |
...
| PR | 内容 | 文件 | 验收 |
|---|---|---|---|
| **E-1** | `ppu_vbl_nmi` 的 `02-vbl_set_time` 「VBL period too long #8」：P4-1 的 VBL cycle 0→1 调整疑似过冲，重新平衡周期计数 | `src/ppu_rendering.cpp:1547-1582` | `01-vbl_basics` 不回归的前提下 `02` 转 PASS |
| **E-2** | 补齐 blargg 诊断输出：`official_only.nes`（停在 4/16）与 `all_instrs.nes` 的 $6004 诊断区为空，需增加运行帧数或调整参数以捕获失败操作码 | `tests/blargg_runner.cpp` | 失败时能打印具体子测试名与操作码 |
| **E-3** | APU `1-len_ctr` 子测试 #7 + `src/sound.cpp:1095` 的 reset 寄存器 FIXME 联合排查（两者疑似同源） | `src/sound.cpp` | 至少产出根因结论；能修则修，不能修则记录为有依据的已知限制 |
...
| # | 标准 | 验证方式 |
|---|---|---|
| 1 | 裸 PowerShell 下 `.\scripts\do_build.ps1 -Config Release` 一次成功 | 无 C1041 / LNK1104；自动选 Ninja |
| 2 | 本地 `ctest -LE perf` 与 CI 结果一致 | 逐项比对 |
| 3 | `lua_bit_test_headless` PASS | Phase B |
| 4 | `kagami_qa_direct_smoke` PASS（不再 `Not Run`） | Phase C |
| 5 | migration matrix 由 CI 产出且 passed 率有据可查 | Phase D |
...
| 风险 | 等级 | 说明 |
|---|---|---|
| C-4 的 LNK2005 重复 std | 🔴 高 | 这是唯一未能通过只读手段排除的链接风险（未运行链接器）。推荐解法（复用根 crate staticlib）可绕开，但需实测 |
| direct 模式的**运行时**正确性 | 🟡 中 | Phase C 全部论证只覆盖链接成功。进程内驱动模拟器是否与 subprocess 模式等价，完全未验证 |
| E-1 VBL 周期调整 | 🟡 中 | 精度问题存在「修好一个测试、弄坏另一个」的经典风险，必须回归全部 `ppu_vbl_nmi` 子测试 |
