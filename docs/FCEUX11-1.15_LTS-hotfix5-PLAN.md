...
逐条对照 `docs/history/FCEUX11-1.15_LTS-hotfix4-PLAN.md` 的 D-1~D-16，在当前代码树上用 `Grep` / `Read` 复现每条修复是否真实落地；再用 `grep -rln hotfix4 src/ scripts/` 列出 hotfix4 实际触碰的全部文件，判断是否触及核心模拟热路径（`src/ppu_rendering.cpp` / `src/ppu.cpp` / `src/x6502.cpp` / `src/apu.cpp` / `src/sound.cpp` / `src/ines.cpp`）。
