...
| 6 | Integration | ⚙️ **战略转向 + T1 实测完成（2026-08-13）** | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层(blargg T1 + nestest T2 + 回归基线 T3 + mapper byte-diff T4 + 8 游戏 smoke T5)。**T1 实测结果**(commit `cb89175`):**81.36%** (144/177),较 v1.16 净改善 +24 PASS / -27 FAIL,0 新增 regression;分项 apu 96.15% / ppu 85.71% / cpu 79.31% / mmc3 33.33%。**phase 6 T1 门槛值 TBD**——3 候选(80% 已过 / 85% 差 6 / 90% 差 16)见 [KagamiQA.md §3.3a](../tech/KagamiQA.md),**下次构建决策**。剩余 work: Step 2-6(47/47 升级 / mapper byte-diff / 8 游戏 smoke / deviations.yaml / 集成)。详细 DoD 见 [phase_6 §7](./phase_6_integration.md)。 | 3 周 |
