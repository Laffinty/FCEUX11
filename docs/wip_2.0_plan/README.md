...
| 6 | Integration | ⚙️ **战略转向（2026-08-13）** + P0/P1 完成 + P2 进行中 | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层(blargg T1 + nestest T2 + 回归基线 T3 + mapper byte-diff T4 + 8 游戏 smoke T5)。Rust core 允许偏离 FCEUX C++ 实现(只要 chip 规范 + KagamiQA 5 层通过);偏离登记 `deviations.yaml`。P0/P1 完成(同 2026-08-12 描述)。P2 进行中:shadow run harness + CPU 差异迭代修复第一~十批已修(cout÷16、VBL-set suppression、frame counter 全功能等真 bug),frame 1-2 真匹配;**剩余 work 转 KagamiQA corpus 补全 + 8 游戏 smoke**。**当前 cpu_match=5/59 baseline 已冻为"开发期回归工具的样本",不再阻塞 phase 6 收口**。详细 DoD 见 [phase_6 §7](./phase_6_integration.md),KagamiQA 5 层 oracle 契约见 [tech/KagamiQA.md](../tech/KagamiQA.md)。 | 3 周 |
