...
| 6 | Integration | ⚙️ 进行中（2026-08-12）：P0 完成 + P1 骨架 + 冒烟测试 | P0 完成：`vnesu11_emulate_frame` 真实实现（4 个 FFI 测试）；`fceu.cpp::Emulate()` shadow 分支接通；CHR 转发（`Bus::setchr1/4/8` → `vnesu11_chr_set_page` → `chr_pages[8]`）；`MapperMetaVtable::tick_irq` 读 C++ CPU IRQ 状态。P1 完成：`vnesu11_shadow.{h,cpp}`（ShadowData + CRC32 + periodic log）；peek/poke/ppu_peek/peek_regs/poke_regs FFI 接线；6 个 system_type 冒烟测试（iNES/VS/FDS/NSF/非法值/NSF帧）；`vn_perf_bench` **743 us/帧 ≈ 1346 FPS**。**本地验证**：`cargo test -p vnesu11` 322 passed / 0 failed / 1 ignored / 0 warnings；VNESU11_CORE=ON/OFF 双配置构建成功。**待办（后续会话/Phase 7）**：端到端 shadow run（kagami-qa + ROM fixtures + 完整 3-tier diff）；FCEUI_* 直接模式垫片；fill_audio 扩展音频；savestate/movie round-trip | 3 周 |
