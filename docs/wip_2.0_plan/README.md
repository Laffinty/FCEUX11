...
| 6 | Integration | ✅ **收口完成（2026-08-13,commit `b687980`）** | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层(blargg T1 + nestest T2 + 回归基线 T3 + mapper byte-diff T4 + 8 游戏 smoke T5)。**DoD 收口实测**:`kagami-qa-runner` 全量 48 项 = 40 PASS / 8 FAIL(advisory) / 0 PASS→FAIL 回归,Grade B;T1 = 81.36%(144/177,+24 vs v1.16,0 新增 regression);shadow baseline cpu_match=5/59(ADR-011 阈值);T4 mapper byte-diff 175/175 PASS;vn_perf_bench 743us/帧 vs C++ 724us/帧(+2.6%);bench_tolerance_test 5/5 PASS(v2.0 baseline + 7% tolerance + 5% headroom)。**T1 门槛选定候选 A(≥ 80%)**(见 [KagamiQA.md §3.3a](../tech/KagamiQA.md))——phase 7 默认 ON 硬门槛 ≥ 85%、phase 8 ≥ 90%、v2.1 ≥ 95%（统一口径见 KagamiQA §3.3）。**关键 fix**:`/FORCE:MULTIPLE` 加到 `tests/CMakeLists.txt::fceux11_add_test_executable`(vnesu11 + fceux11_rust 双 Rust std 嵌入)、FDS golden 再生、bench tolerance 升级、shadow runner 退出码按 ADR-011 阈值。**新增交付**:`docs/wip_2.0_plan/deviations.yaml`(6 条 D-B 登记)、`scripts/smoke_run_games.ps1`(T5 骨架)、`tests/tests.json::shadow_run_cpu_smoke`(T3 gate)。详细收口日志见 [phase_6 §7.6](./phase_6_integration.md)。 | 3 周 |
