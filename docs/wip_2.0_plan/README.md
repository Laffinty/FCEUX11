...
| 6 | Integration | ⚠️ **收口进行中（2026-08-14 DMC 收口会话）** | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层。**诚实 Rust T1 = 140/177 = 79.1%**（`VNESU11_RUST_PRIMARY=1` 实测，较上一会话 135/177 净 +5）。本会话完成 DMC 完整实现（DMA 取样/输出单元/IRQ/loop/$4010-$4015 状态机）+ NOP abs,X 页跨 dummy read。**剩余 37 失败**：sprdma×2（DMC+Sprite DMA 仲裁）/ CPU 9（含 cpu_dummy_writes_ppu 等待 VBL 卡死）/ MMC3 12（7 缺 A12 时钟）/ PPU 13（vbl 需 dot 粒度）。**接续指引见 [phase_6 §9.1.0](./phase_6_integration.md)（已更新）**。 | 3 周 |
