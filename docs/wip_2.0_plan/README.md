...
| 6 | Integration | ⚙️ 进行中（2026-08-12）：P0/P1 完成 + P2 shadow run harness + 差异迭代修复进行中 | P0 完成：`vnesu11_emulate_frame` 真实实现（4 个 FFI 测试）；`fceu.cpp::Emulate()` shadow 分支接通；CHR 转发（`Bus::setchr1/4/8` → `vnesu11_chr_set_page` → `chr_pages[8]`）；`MapperMetaVtable::tick_irq` 读 C++ CPU IRQ 状态。P1 完成：`vnesu11_shadow.{h,cpp}`（ShadowData + CRC32 + periodic log）；peek/poke/ppu_peek/peek_regs/poke_regs FFI 接线；6 个 system_type 冒烟测试；`vn_perf_bench` **743 us/帧 ≈ 1346 FPS**。P2 完成：shadow run 端到端 harness（`kagami_qa_shadow_run_runner`）+ CPU 差异迭代修复第一~九批——$2002 VBlank 路由、DEY/TAY 周期表对齐、段预算对齐（VBlank 341 + -1 段 0）、per-instruction APU tick + IRQ 路由、frame counter 全功能（NTSC/PAL × 4/5-step，周期 29830）+ 状态同步、PPU 状态同步、**预算单位补偿（×3 dots/cycle，frame counter 相位差 5200 → 8-55）**、frame counter IRQ 跨 wrap 保持（C++ FrameIRQEnd mode-0 对齐）。**本地验证**：`cargo test -p vnesu11` 全模块通过（lib 192 + apu 24 + ppu 33 + mapper 12 + system_type 等）；cpu_dummy_reads shadow frame 1-2 完全匹配。**待办（后续会话/Phase 7）**：frame 3+ 相位残差（IRQ/NMI 服务时序 + PPU v/t/x/w 同步）；APU 5 通道状态同步；FCEUI_* 直接模式垫片；fill_audio 扩展音频；savestate/movie round-trip | 3 周 |
