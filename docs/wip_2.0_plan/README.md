...
| 6 | Integration | ⚠️ **复核中（2026-08-13 会话末修订,commit `443f1b1`）** | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层。**⚠️ 重大修订**：本会话发现并修复了根本构建 bug（`VNESU11_CORE_ENABLED` 从未真正生效，此前 T1 实测的是 C++ newppu）+ CPU 栈序 bug（`push/pop` 顺序反了，使 T1 假阳性）。**诚实 Rust T1 = 111/177 = 62.7%**（`VNESU11_RUST_PRIMARY=1` 实测），此前 87%/81.36% 均不可用。**本会话已提交 4 个 commit**：`d7324e3` 构建宏+CPU/PPU 基础修复、`33d95cf` 正确栈+VBlank+HBlank+dummy read、`d8ef1c8` RMW dummy read、`443f1b1` 完整软复位。**剩余 66 失败**：APU ~16 / CPU ~23 / MMC3 18（A12 时钟缺）/ PPU ~10。**接续指引见 [phase_6 §9.1.0](./phase_6_integration.md)（已重写）**。 | 3 周 |
