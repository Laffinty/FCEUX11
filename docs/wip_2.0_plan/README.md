...
| 6 | Integration | ⚠️ **收口进行中（2026-08-13 收口会话,改动未提交）** | **战略转向**：byte-level shadow match 不再是精度判据,精度 oracle 升格为 [KagamiQA](../tech/KagamiQA.md) 5 层。**诚实 Rust T1 = 135/177 = 76.3%**（`VNESU11_RUST_PRIMARY=1` 实测，较会话开始 111/177 = 62.7% 净 +24）。本会话修复 APU 长度计数/帧计数 IRQ latch + CPU 非法 opcode（ALR/ANC/SYA/SXA/SHA 等）+ RMW 寻址 + NOP abs/abs,X + CLI/SEI/PLP 中断延迟。**剩余 42 失败**：APU 8（需完整 DMC）/ CPU 11（中断采样等）/ MMC3 12（7 个仍缺 A12 时钟）/ PPU 13（vbl 需 dot 粒度）。**接续指引见 [phase_6 §9.1.0](./phase_6_integration.md)（已更新）**。 | 3 周 |
