# Phase 7 · 默认切换 & 灰度发布（**已被 Phase 7a / 7b 取代**）

> ⚠️ **2026-08-14 路径 B 决议**：本文件已被拆分为独立排期的两个阶段，**本文件停止维护**：
> - **[Phase 7a · v2.0 默认切换（精度冻结 80.8%）](./phase_7a_default_switch.md)**——2-3 周。CMake 默认翻转、CI 双构、Release tag `v2.0.0`。**不阻塞精度攻关**。
> - **[Phase 7b · 精度攻关（T1 80.8% → 85%）](./phase_7b_accuracy.md)**——4-5 周。MMC3 IRQ 0x02 + CPU 中断时序 → T1 ≥ 150/177。Release tag `v2.0.1` 或 `v2.1`。
>
> 原计划假设 T1 81.36% → 85% 是 2 周内可达，实际剩余 7 PASS 缺口（A12 clocking deep model + PPU dot 粒度 + DMA 仲裁）需多周 deep model 改动。**精度攻关与发布解耦**让 v2.0 不被 deep model 改动阻塞。
>
> ---
>
> **历史内容保留**：原 Phase 7 计划如下（已被取代，仅作历史档案）。
>
