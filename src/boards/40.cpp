...
void Mapper40_Init(CartInfo *info) {
	info->Reset = M40Reset;
	info->Power = M40Power;
	g_cpu.set_map_irq_hook(M40IRQHook);
...
namespace fceu11 {
namespace {
static MapperEntryRegister kMapper40Register{
    MapperEntry{40, "SMB2j FDS", &Mapper40_Init,
        [](Bus& bus) { return std::make_unique<Mapper40Cart>(bus); }}
};
}  // namespace
}  // namespace fceu11
