	...
	if(!fceuindbg) {
		if(!(A & 1)) {
			if((EXPREGS[0] & 0xE0) == 0xC0) {
				EXPREGS[1] = fceu11::g_bus.read(0x6a);	// program can latch some data from the BUS, but I can't say how exactly,
			} else {							// without more euipment and skills ;) probably here we can try to get any write
				EXPREGS[2] = fceu11::g_bus.read(0xff);	// before the read operation
			}
