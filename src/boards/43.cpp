...
void Mapper43_Init(CartInfo *info) {
	info->Reset = M43Reset;
	info->Power = M43Power;
	g_cpu.set_map_irq_hook(M43IRQHook);
...
namespace fceu11 {
namespace {
static MapperEntryRegister kMapper43Register{
    MapperEntry{43, "SMB2j FDS 43", &Mapper43_Init,
        [](Bus& bus) { return std::make_unique<Mapper43Cart>(bus); }}
};
}  // namespace
}  // namespace fceu11
