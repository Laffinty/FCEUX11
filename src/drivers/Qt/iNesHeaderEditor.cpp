	...
	: QDialog( parent )
{
	...
	FCEU_strlcpy( stmp, sizeof(stmp), "0 B");
...
bool iNesHeaderEditor_t::WriteHeaderData(iNES_HEADER* header)
{
	...
	if (ines20) 
	{
		FCEU_strlcpy(buf, sizeof(buf), mapperSubEdit->text().toStdString().c_str() );
		int submapper;
	...
	if (ines20)
	{
		FCEU_strlcpy(buf, sizeof(buf), miscRomsEdit->text().toStdString().c_str() );
		int misc_roms = 0;
