...
void GamePadConfDialog_t::refreshKeyBindTree( bool reset )
{
	...
	for (it=GamePad[portNum].gpKeySeqList.begin(); it!=GamePad[portNum].gpKeySeqList.end(); it++)
	{
		...
		if ( btnName[0][0] != 0 )
		{
			safe_strcat( btnSeq, sizeof(btnSeq), btnName[0] );
			safe_strcat( btnSeq, sizeof(btnSeq), " + ");
		}
		...
		if ( btnName[1][0] != 0 )
		{
			safe_strcat( btnSeq, sizeof(btnSeq), btnName[1] );
		}
...
void GamePadConfDialog_t::updateCntrlrDpy(void)
{
	...
	for (int i = 0; i < GAMEPAD_NUM_BUTTONS; i++)
	{
		...
		{
			FCEU_strlcpy( keyNameStr, sizeof(keyNameStr), ButtonName(&GamePad[portNum].bmap[configIndex][i]));
		}
