...
// hotfix3 B-5a: was `void (*&)(int)` returning a true reference. The
// atomic-backed map_irq_hook_ref() now returns a Cpu::RefProxy value; the
// view accessor returns the proxy by value too, preserving the legacy
// `CpuView::irq_hook() = SomeFunc;` pattern via proxy operator=.
Cpu::RefProxy CpuView::irq_hook() noexcept { return fceu11::cpu_instance().map_irq_hook_ref(); }
