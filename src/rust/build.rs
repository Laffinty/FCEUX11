...
fn main() {
    ...
    let status = Command::new(&cbindgen)
        .args(&["--crate", "fceux11-debug", "--config", &config, "--output", &debug_out])
        .current_dir(&crate_dir)
        .status()
    ...
    let status = Command::new(&cbindgen)
        .args(&["--crate", "fceux11-lua", "--config", &config, "--output", &lua_out])
        .current_dir(&crate_dir)
        .status()
    ...
    let status = Command::new(&cbindgen)
        .args(&["--crate", "fceux11-core", "--config", &config, "--output", &core_out])
        .current_dir(&crate_dir)
        .status()
    ...
    merge_headers(&crate_dir, &utils_out, &media_out, &formats_out, &debug_out, &lua_out, &core_out);
...
fn merge_headers(crate_dir: &str, utils_path: &str, media_path: &str, formats_path: &str, debug_path: &str, lua_path: &str, core_path: &str) {
