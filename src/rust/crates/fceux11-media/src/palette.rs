...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_palette_calc_ntsc(tint: c_int, hue: c_int, out: *mut Pal) {
    if out.is_null() {
        return;
    }
    ...
    for x in 0..=3 {
        for (z, &cols_z) in COLS.iter().enumerate() {
            let mut s = s_base;
            let mut luma = BR2[x];
            if z == 0 {
                s = 0.0;
            ...
            let theta = std::f64::consts::PI
                * ((cols_z as f64 * 10.0 + (hue as f64 / 2.0) + 300.0) / 180.0);
...
fn apply_deemphasis_bisqwit(entry: i32, r: &mut u8, g: &mut u8, b: &mut u8) {
    ...
    for pass in 0..2 {
        ...
        for p in 0..12 {
            ...
            if pass == 1
                && (((entry & 0x40) != 0 && bisqwit_wave(p, 12) != 0)
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_palette_apply_deemphasis(src: *const Pal, dst: *mut Pal) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_palette_make_grayscale(src: *const Pal, dst: *mut Pal) {
    if src.is_null() || dst.is_null() {
        return;
    }
    ...
    for x in 0..512 {
        let gray =
            (src[x].r as f32 * 0.299 + src[x].g as f32 * 0.587 + src[x].b as f32 * 0.114) as u8;
        dst[x].r = gray;
        dst[x].g = gray;
        dst[x].b = gray;
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_palette_draw_control_bars(
    xbuf: *mut u8,
    width: c_int,
    which: c_int,
) {
