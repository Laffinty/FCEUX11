...
impl FceuInesHeader {
    ...
    pub fn cleanup(&mut self) {
        ...
        // Offset 0x7 .. 0xF
        if bytes[0x7..0xF].starts_with(b"DiskDude")
            || bytes[0x7..0x10].starts_with(b"demiforce")
        {
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_ines_header_cleanup(header_bytes: *mut u8) {
...
pub extern "C" fn fceux11_rust_ines_mapper_name(mapper_no: i32) -> *const c_char {
    match ines_data::MAPPER_NAMES.iter().find(|(n, _)| *n == mapper_no) {
        Some(entry) => {
            let name: &str = entry.1;
            // For simplicity we use a thread-local buffer.
            thread_local! {
                static BUF: std::cell::RefCell<[u8; 256]> = std::cell::RefCell::new([0u8; 256]);
            }
            BUF.with(|buf| {
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_ines_lookup_input_crc(
    crc32: u32,
    out_input1: *mut i32,
    out_input2: *mut i32,
    out_inputfc: *mut i32,
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_ines_lookup_input_nes20(
    expansion: u8,
    out_input1: *mut i32,
    out_input2: *mut i32,
    out_inputfc: *mut i32,
    ...
    match ines_data::INPSEL_NES20.iter().find(|e| e.expansion_id == expansion) {
        Some(e) => {
            unsafe {
                *out_input1 = e.input1;
                *out_input2 = e.input2;
...
pub extern "C" fn fceux11_rust_ines_check_bad(md5partial: u64) -> *const c_char {
    match ines_data::BAD_ROMS.iter().find(|e| e.md5partial == md5partial) {
        Some(e) => {
            thread_local! {
                static BUF: std::cell::RefCell<[u8; 256]> = std::cell::RefCell::new([0u8; 256]);
            }
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_ines_check_hinfo(
    crc32: u32,
    partialmd5: u64,
...
mod tests {
    ...
    fn test_lookup_input_crc_duck_hunt() {
        ...
        let mut i2 = 0;
        let mut ifc = 0;
        let found = fceux11_rust_ines_lookup_input_crc(0x24598791, &mut i1, &mut i2, &mut ifc);
        assert_eq!(found, 1);
    ...
    fn test_lookup_input_crc_not_found() {
        ...
        let mut i2 = 0;
        let mut ifc = 0;
        let found = fceux11_rust_ines_lookup_input_crc(0xDEADBEEF, &mut i1, &mut i2, &mut ifc);
        assert_eq!(found, 0);
    }
    ...
    fn test_lookup_input_nes20_standard() {
        ...
        let mut eopt = 0;
        let mut vsc = 0;
        let found = fceux11_rust_ines_lookup_input_nes20(
    ...
    fn test_check_hinfo_force_battery() {
        ...
        fceux11_rust_ines_check_hinfo(0, 0xc04361e499748382_u64, &mut res);
    ...
    fn test_check_hinfo_mapper_correction() {
        ...
        fceux11_rust_ines_check_hinfo(0xfcdaca80, 0, &mut res);
