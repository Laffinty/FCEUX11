...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_header_validate(
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_compute_banks(
    ...
    let nsf_max_bank = uppow2(
        ((nsf_size.wrapping_add((load_addr & 0xfff) as u32)).wrapping_add(4095)) / 4096,
    );
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_patch_nsfrom(
    nsfrom: *mut u8,
    nsfrom_len: usize,
    init_addr: u16,
    play_addr: u16,
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_chip_name(sound_chip: u8, out_mask: *mut u8) -> *const c_char {
    for (i, &name) in CHIP_NAMES.iter().enumerate() {
        let mask = 1 << i;
        if sound_chip & mask != 0 {
            if !out_mask.is_null() {
                unsafe { *out_mask = mask; }
            }
            thread_local! {
                static BUF: std::cell::RefCell<[u8; 64]> = std::cell::RefCell::new([0u8; 64]);
            }
            return BUF.with(|buf| {
...
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_change_song(
    current: i32,
    amount: i32,
    total: i32,
    out_reload: *mut u8,
...
/// Returns `TotalSongs`.
#[unsafe(no_mangle)]
pub extern "C" fn fceux11_rust_nsf_get_info(
    header: *const FceuNsfHeader,
    name: *mut u8,
    artist: *mut u8,
    copyright: *mut u8,
...
mod tests {
    ...
    fn test_header_validate_ok() {
        ...
        let mut init = 0u16;
        let mut play = 0u16;
        assert!(fceux11_rust_nsf_header_validate(&mut h, &mut load, &mut init, &mut play));
    ...
    fn test_header_validate_null_safety() {
        assert!(!fceux11_rust_nsf_header_validate(
            std::ptr::null_mut(),
            std::ptr::null_mut(),
            std::ptr::null_mut(),
            std::ptr::null_mut()
    ...
    fn test_compute_banks_no_bankswitch() {
        ...
        let mut bson = 0u8;
        let mut banks = [0u8; 8];
        assert!(fceux11_rust_nsf_compute_banks(
            &h,
            4096,
            &mut max_bank,
            &mut bson,
    ...
    fn test_patch_nsfrom() {
        ...
        assert!(fceux11_rust_nsf_patch_nsfrom(
            nsfrom.as_mut_ptr(),
            nsfrom.len(),
            0xABCD,
            0x1234
    ...
    fn test_patch_nsfrom_not_found() {
        let mut buf = [0u8; 10];
        assert!(!fceux11_rust_nsf_patch_nsfrom(buf.as_mut_ptr(), buf.len(), 0x1234, 0x5678));
    }

    #[test]
    fn test_chip_name() {
        let mut mask = 0u8;
        assert!(!fceux11_rust_nsf_chip_name(0x01, &mut mask).is_null());
        assert_eq!(mask, 0x01);
        assert!(!fceux11_rust_nsf_chip_name(0x02, &mut mask).is_null());
        assert_eq!(mask, 0x02);
        assert!(!fceux11_rust_nsf_chip_name(0x04, &mut mask).is_null());
        assert_eq!(mask, 0x04);
        assert!(!fceux11_rust_nsf_chip_name(0x08, &mut mask).is_null());
        assert_eq!(mask, 0x08);
        assert!(!fceux11_rust_nsf_chip_name(0x10, &mut mask).is_null());
        assert_eq!(mask, 0x10);
        assert!(!fceux11_rust_nsf_chip_name(0x20, &mut mask).is_null());
        assert_eq!(mask, 0x20);
        assert!(fceux11_rust_nsf_chip_name(0x00, &mut mask).is_null());
        assert_eq!(mask, 0);
        assert!(fceux11_rust_nsf_chip_name(0x40, &mut mask).is_null());
        assert_eq!(mask, 0);
    }
    ...
    fn test_change_song() {
        let mut reload = 0u8;
        assert_eq!(fceux11_rust_nsf_change_song(5, 1, 10, &mut reload), 6);
        assert_eq!(reload, 0xFF);
        assert_eq!(
            fceux11_rust_nsf_change_song(1, -1, 10, std::ptr::null_mut()),
            1
        );
        assert_eq!(
            fceux11_rust_nsf_change_song(10, 1, 10, std::ptr::null_mut()),
            10
        );
        assert_eq!(
            fceux11_rust_nsf_change_song(5, 10, 10, std::ptr::null_mut()),
            10
        );
        assert_eq!(
            fceux11_rust_nsf_change_song(5, -10, 10, std::ptr::null_mut()),
    ...
    fn test_get_info() {
        ...
        let total = fceux11_rust_nsf_get_info(
            &h,
            name.as_mut_ptr(),
            artist.as_mut_ptr(),
    ...
    fn test_get_info_null_safety() {
        ...
        h.total_songs = 3;
        assert_eq!(
            fceux11_rust_nsf_get_info(&h, std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), 0),
            3
        );
    }
}
