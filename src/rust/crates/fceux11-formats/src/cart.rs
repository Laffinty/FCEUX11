...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_cart_compute_rom_sizes(
...
/// point to valid memory regions of the expected size for the duration of the call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_cart_battery_save(
    path: *const c_char,
...
/// point to valid memory regions of the expected size for the duration of the call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_cart_battery_load(
    path: *const c_char,
    ...
    for entry in entries {
        if !entry.bufptr.is_null() && entry.buflen > 0 {
            let buf =
                unsafe { std::slice::from_raw_parts_mut(entry.bufptr, entry.buflen as usize) };
...
/// point to valid memory regions of the expected size for the duration of the call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fceux11_rust_cart_battery_clear(
    entries: *const FceuSaveGameEntry,
    count: usize,
) {
    if entries.is_null() || count == 0 {
...
mod tests {
    ...
    fn test_rom_sizes_ines1_basic() {
        unsafe {
            ...
            assert!(unsafe { fceux11_rust_cart_compute_rom_sizes(&header, false, &mut out) });
    ...
    fn test_rom_sizes_ines2_simple() {
        unsafe {
            ...
            assert!(unsafe { fceux11_rust_cart_compute_rom_sizes(&header, true, &mut out) });
    ...
    fn test_chrram_size_mapper_defaults() {
        unsafe {
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(0, false, 0, 0, false),
                8 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(13, false, 0, 0, false),
                16 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(6, false, 0, 0, false),
                32 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(29, false, 0, 0, false),
                32 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(30, false, 0, 0, false),
                32 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(45, false, 0, 0, false),
                32 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(96, false, 0, 0, false),
                32 * 1024
            );
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(176, false, 0, 0, false),
                128 * 1024
            );
        }
    }
    ...
    fn test_chrram_size_with_vrom() {
        unsafe {
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(0, false, 0, 0, true),
                -1
            );
        }
    }
    ...
    fn test_chrram_size_nes2() {
        unsafe {
            assert_eq!(
                fceux11_rust_cart_compute_chrram_size(0, true, 4096, 8192, false),
                12288
            );
        }
    }
    ...
    fn test_battery_save_load_roundtrip() {
        unsafe {
            ...
            let entries_save = [
                FceuSaveGameEntry {
                    bufptr: buf1.as_mut_ptr(),
                    buflen: buf1.len() as u32,
                },
                FceuSaveGameEntry {
                    bufptr: buf2.as_mut_ptr(),
                    buflen: buf2.len() as u32,
                },
            ];

            let path = CString::new(tmp.to_str().unwrap()).unwrap();
            assert!(unsafe {
                fceux11_rust_cart_battery_save(
                    path.as_ptr(),
                    entries_save.as_ptr(),
                    entries_save.len(),
                )
            ...
            let mut entries_load = [
                FceuSaveGameEntry {
                    bufptr: buf1r.as_mut_ptr(),
                    buflen: buf1r.len() as u32,
                },
                FceuSaveGameEntry {
                    bufptr: buf2r.as_mut_ptr(),
                    buflen: buf2r.len() as u32,
                },
            ];

            assert!(unsafe {
                fceux11_rust_cart_battery_load(
                    path.as_ptr(),
                    entries_load.as_mut_ptr(),
                    entries_load.len(),
                )
            });
            ...
            // Clear
            unsafe { fceux11_rust_cart_battery_clear(entries_load.as_ptr(), entries_load.len()) };
    ...
    fn test_battery_null_safety() {
        unsafe {
            assert!(!unsafe {
                fceux11_rust_cart_battery_save(std::ptr::null(), std::ptr::null(), 0)
            });
            assert!(!unsafe {
                fceux11_rust_cart_battery_load(std::ptr::null(), std::ptr::null_mut(), 0)
            });
            // clear with null / zero count should not crash
            unsafe { fceux11_rust_cart_battery_clear(std::ptr::null(), 0) };
        }
    }
}
