#!/usr/bin/env python3
"""Disassemble the copied code ($EA87-$EB86 in bank 15) that runs at $0700."""
data = open("tests/fixtures/blargg/cpu/instr_v5_all.nes", "rb").read()
prg = 16384

def at(pc):
    return data[16 + 15 * prg + (pc - 0xC000)]
...
mn = {
    ...
    0x91: ("STA ($", 2), 0x91: ("STA ($", 2), 0x91: ("STA ($", 2),
    ...
    0xCE: ("DEC $", 3), 0x8D: ("STA $", 3), 0x86: ("STX $", 2), 0xA6: ("LDX $", 2),
}

print("Copied code (source $EA87-$EB86 -> runs at $0700):")
pc = 0xEA87
n = 0
while pc < 0xEB87 and n < 60:
    op = at(pc)
    ...
    if name.endswith("$") and size == 3:
        ...
        opstr = name
    # Branch targets
    if name.endswith("$") and size == 2 and op in (0xD0, 0xF0, 0xB0, 0x90, 0x10, 0x30, 0x50, 0x70):
        off = at(pc + 1)
        tgt = (pc + 2 + (off - 256 if off >= 0x80 else off)) & 0xFFFF
