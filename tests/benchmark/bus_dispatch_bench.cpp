// FCEUX11 v1.14 Anvil — Bus Dispatch Benchmark
// Measures ARead/BWrite indirect call overhead by emulating 60 frames of
// CPU-intensive ROM (nestest.nes) with sound disabled.
// v1.14: isolated Bus dispatch overhead benchmark for §14.1.
...
static double benchmark_bus_frames(int frames)
{
    ...
    // Sound disabled — isolate CPU+Bus dispatch path.
...
int main(int argc, char** argv)
{
    ...
    if (!json_mode) {
        printf("=== FCEUX11 Bus Dispatch Benchmark (v1.14 Anvil) ===\n");
        printf("ROM: %s\n", ROM_PATH);
    ...
    for (int i = 0; i < iterations; ++i) {
        double ms = benchmark_bus_frames(FRAMES);
        if (ms < 0.0) return 1;
        if (!json_mode) {
    ...
    if (json_mode) {
        printf(
            "{"
            "\"name\":\"bench_bus_dispatch\","
            "\"binary\":\"fceux11_bench_bus_dispatch\","
            "\"rom\":\"%s\","
            "\"frames_per_iter\":%d,"
            "\"iterations\":%d,"
            ...
            "\"stddev_ms\":%.3f,"
            "\"stddev_pct\":%.1f,"
            "\"unit\":\"milliseconds for %d frames (Bus dispatch)\""
            "}\n",
