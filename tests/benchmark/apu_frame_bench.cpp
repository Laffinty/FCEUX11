// FCEUX11 v1.14 Anvil — APU Frame Benchmark
// Measures APU audio generation throughput by emulating 60 frames with sound
// enabled on an MMC3 mapper ROM (same ROM as bench_full_frame).
// v1.14: isolated APU-focused benchmark for §14.1.
...
int main(int argc, char** argv)
{
    ...
    if (!json_mode) {
        printf("=== FCEUX11 APU Frame Benchmark (v1.14 Anvil) ===\n");
        printf("ROM: %s\n", ROM_PATH);
    ...
    if (json_mode) {
        printf(
            "{"
            "\"name\":\"bench_apu_frame\","
            "\"binary\":\"fceux11_bench_apu_frame\","
            "\"rom\":\"%s\","
            "\"frames_per_iter\":%d,"
            "\"iterations\":%d,"
            ...
            "\"stddev_ms\":%.3f,"
            "\"stddev_pct\":%.1f,"
            "\"unit\":\"milliseconds for %d frames (APU)\""
            "}\n",
