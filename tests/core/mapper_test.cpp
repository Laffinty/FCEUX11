...
void test_mapper_lifecycle_pointers(TestContext& ctx) {
    // currCartInfo is the global CartInfo; the Power/Reset/Close
    // function pointers must be populated for all 4 mappers.
    ...
    FCEU11_EXPECT(&ctx, currCartInfo->Reset != nullptr, "currCartInfo->Reset populated");
