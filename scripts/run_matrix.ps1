...
# src/rust/crates/f11qa/build.rs (report/matrix.rs uses option_env!).
...
#   cargo build --release -p f11qa --bin f11qa-runner

& "$ROOT\src\rust\target\x86_64-pc-windows-msvc\release\f11qa-runner.exe" `
    --manifest "$ROOT\tests\tests.json" `
    --bin-dir  "$ROOT\build-c1\tests" `
    --baseline  "$ROOT\build-c1\f11qa_baseline.json" `
    --output    "$ROOT\build-c1\f11qa_migration_matrix.json"
